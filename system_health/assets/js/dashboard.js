...
//These error widgets include both the ERROR COUNT and the RECENT ERROR LOGS
    function errors() {
        $.getJSON("/errors", function(result){
                ...
                var day_link = result["kibana_link"] + "/s/skywatch-ai/app/kibana#/discover?_g=(refreshInterval:(pause:!t,value:0),time:(from:now-24h,mode:quick,to:now))&_a=(columns:!(RequestPath,Username,message,StatusCode),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:'" + result['kibana_index'] + "',key:level,negate:!t,params:(query:Warning,type:phrase),type:phrase,value:Warning),query:(match:(level:(query:Warning,type:phrase))))),index:'" + result['kibana_index'] + "',interval:auto,query:(language:lucene,query:error),sort:!('@timestamp',desc))";
                var week_link = result["kibana_link"] + "/s/skywatch-ai/app/kibana#/discover?_g=(refreshInterval:(pause:!t,value:0),time:(from:now-7d,mode:quick,to:now))&_a=(columns:!(RequestPath,Username,message,StatusCode),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:'" + result['kibana_index'] + "',key:level,negate:!t,params:(query:Warning,type:phrase),type:phrase,value:Warning),query:(match:(level:(query:Warning,type:phrase))))),index:'" + result['kibana_index'] + "',interval:auto,query:(language:lucene,query:error),sort:!('@timestamp',desc))";
                var month_link = result["kibana_link"] + "/s/skywatch-ai/app/kibana#/discover?_g=(refreshInterval:(pause:!t,value:0),time:(from:now-30d,mode:quick,to:now))&_a=(columns:!(RequestPath,Username,message,StatusCode),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,index:'" + result['kibana_index'] + "',key:level,negate:!t,params:(query:Warning,type:phrase),type:phrase,value:Warning),query:(match:(level:(query:Warning,type:phrase))))),index:'" + result['kibana_index'] + "',interval:auto,query:(language:lucene,query:error),sort:!('@timestamp',desc))";
             ...
             fill_counter_widget(document.getElementById("errors_last_month"),
                                ...
                                "Errors Last Month:",
                                null);
             fill_error_widgets(result['most_recent_logs'], result["kibana_link"], result["kibana_index"]);
        });
    };

//Here the error widgets take on a different format to the counter widget, so a new function is made to modify the format
    function fill_error_widgets(errors, country_link, index) {
        ...
        for(i=0; i < len; i++) {
            ...
            var error = errors[i];
            var id = error['id'];
            var link = country_link + "/s/skywatch-ai/app/kibana#/context/" +
                        index + "/General/" + id + "?_a=(columns:!(RequestPath,Username,message,StatusCode),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!t,index:\'" +
